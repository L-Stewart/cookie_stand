...
for(var i = 0; i < 15; i++){
  aCookies.push(Math.floor(alki.averageSale * alki.random(alki.min, alki.max)));
}
...
// Create the timeList element:
  ...
  // Create the timeList item:
    ...
    // Set its contents:
    item.appendChild(document.createTextNode(options.time[i], options.customers[i]));
...
document.getElementById('fNp').appendChild(makeUL(options.time, options.customers));
